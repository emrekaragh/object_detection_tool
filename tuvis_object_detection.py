...
class SSDHandler(DetectorHandler):
    def __init__(self):
        ...
        self.PATH_TO_TRAIN_PY = os.path.abspath(os.path.join(__file__,'../','SSD','scripts','train.py'))
        self.PATH_TO_PREDICT_PY = os.path.abspath(os.path.join(__file__,'../','SSD','scripts','prediction.py'))
...
class MrcnnHandler(DetectorHandler):
    ...
    def do_train(self, args=None):
        ...
        print('args:',(['python', self.PATH_TO_TRAIN_PY] + args))
        train_check = spRun((['python', self.PATH_TO_TRAIN_PY] + args))
        try:
            train_check.check_returncode()
        except:
...
class RetinaNetHandler(DetectorHandler):
    def __init__(self):
        ...
        self.PATH_TO_RESNET50_WE      GHTS = os.path.abspath(os.path.join(__file__,'../','retinanet','resnet50_coco_best_v2.1.0.h5'))
    ...
    def trainDefaultArgumentsSetter(self, args):
        ...
        if '--weights' not in args and ' --backbone' not in args:
                args.insert(0, (' ' + self.PATH_TO_RESNET50_WE      GHTS))
